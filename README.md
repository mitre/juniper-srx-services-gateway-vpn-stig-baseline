# Juniper SRX Services Gateway Virtual Private Network (VPN) Security Technical Implementation Guide
This InSpec Profile was created to facilitate testing and auditing of `Juniper SRX Services Gateway VPN`
...
The Juniper SRX Services Gateway VPN STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Table of Contents
=================
- [Juniper SRX Services Gateway Virtual Private Network (VPN) Security Technical Implementation Guide](#juniper-srx-services-gateway-virtual-private-network-vpn-security-technical-implementation-guide)
...
Requirements associated with the Juniper SRX Services Gateway VPN STIG are derived from the
...
The Juniper SRX Services Gateway VPN STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the Juniper SRX Services Gateway VPN STIG profile.
...
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/juniper-srx-services-gateway-vpn-stig-baseline.git --input-file=<your_inputs_file.yml> -t juniper://username@device.example.com --password 'secret' --reporter=cli json:<your_results_file.json>

# Using `juniper` transport with bastion host
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/juniper-srx-services-gateway-vpn-stig-baseline.git --input-file=<your_inputs_file.yml> -t juniper://username@device.example.com --password 'secret' --bastion-host='jump-host-name' --reporter=cli json:<your_results_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/juniper-srx-services-gateway-vpn-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive juniper-srx-services-gateway-vpn-stig-baseline
...
```bash
cd juniper-srx-services-gateway-vpn-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive juniper-srx-services-gateway-vpn-stig-baseline --overwrite
